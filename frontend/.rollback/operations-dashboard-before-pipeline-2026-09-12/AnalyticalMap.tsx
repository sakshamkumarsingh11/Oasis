...
export function AnalyticalMap({ data }: { data: DashboardPayload }) {
  ...
  useEffect(() => {
    ...
    map.on('load', () => {
      ...
      const shouldAnimatePipeline = runVersion > playedRun.current;
      if (shouldAnimatePipeline) {
        playedRun.current = runVersion;
  ...
  }, [data, select, visualMode, runVersion]);
